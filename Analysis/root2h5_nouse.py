...
def root2hdf5_Points_detsim_v3 (batch_size, tree, start_event, out_name):##add 1D hist of hittime_cor
    ...
    br_initX    = rt.std.vector('float')()
    br_initY    = rt.std.vector('float')()
    br_initZ    = rt.std.vector('float')()
    br_initPX   = rt.std.vector('float')()
    br_initPY   = rt.std.vector('float')()
    br_initPZ   = rt.std.vector('float')()
    br_edep     = rt.std.vector('float')()
    br_edepX    = rt.std.vector('float')()
    br_edepY    = rt.std.vector('float')()
    br_edepZ    = rt.std.vector('float')()

    tree.SetBranchAddress("gen_InitX" , br_initX )
    tree.SetBranchAddress("gen_InitY" , br_initY )
    tree.SetBranchAddress("gen_InitZ" , br_initZ )
    tree.SetBranchAddress("gen_InitPX", br_initPX)
    tree.SetBranchAddress("gen_InitPY", br_initPY)
    tree.SetBranchAddress("gen_InitPZ", br_initPZ)
    tree.SetBranchAddress("sim_Qedep" , br_edep  )
    tree.SetBranchAddress("sim_QedepX", br_edepX )
    tree.SetBranchAddress("sim_QedepY", br_edepY )
    tree.SetBranchAddress("sim_QedepZ", br_edepZ )
    ...
    for ie in range(start_event, batch_size+start_event):
        ...
        tmp_simy = 0
        tmp_simz = 0
        for i in range(len(br_edep)):
            tmp_simE += br_edep[i]
            print('i=',i,',edepX=',br_edepX[i],',edep=',br_edep[i])
            tmp_simx += br_edepX[i]*br_edep[i]
            tmp_simy += br_edepY[i]*br_edep[i]
            tmp_simz += br_edepZ[i]*br_edep[i]
        edep = tmp_simE 
        edepX = tmp_simx/edep
        edepY = tmp_simy/edep
        edepZ = tmp_simz/edep
