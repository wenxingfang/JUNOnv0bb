...
def doAna(df_data, tag='train') :
    ...
    for i in range(df_data.shape[0]):
        ...
        tmp_pred = df_data[i,-3:]##last 3 values, real_tag, 2 pred
        if tmp_pred[0]==0:
            h_pred_sig.Fill(tmp_pred[1])
            h_E_sig.Fill(tmp_edep)
            h_R_sig.Fill(tmp_R)
        elif tmp_pred[0]==1:
            h_pred_bkg.Fill(tmp_pred[1])
    ...
    y_pred = tmp_pred[:,1]
    draw_roc(y_true=y_true, y_pred=y_pred, leg_name='example estimator', out_name='roc')
        
    return [h_pred_sig,h_pred_bkg,h_E_sig,h_E_bkg,h_R_sig,h_R_bkg]
...
if __name__ == '__main__':
    ...
    #ftrain = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNet_train_sim.txt'
    #ftrain = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNetTF_train_sim.txt'
    #ftrain = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNetTF_train_sim_moreTrain.txt'
    #ftrain = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNet_train_sim_moreTrain.txt'
    ftrain = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_TF_train_moreTrain_sim.txt'
    train_files = readfiles(ftrain)
    #ftest = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNet_test_sim.txt'
    #ftest = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNetTF_test_sim.txt'
    #ftest = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNetTF_test_sim_moreTrain.txt'
    #ftest = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_cls_PNet_test_sim_moreTrain.txt'
    ftest = '/hpcfs/juno/junogpu/fangwx/FastSim/JUNO/nv0bb_cls/pred_TF_test_moreTrain_sim.txt'
    test_files = readfiles(ftest)
    df_data_train = readdf(train_files)
    df_data_test  = readdf(test_files )

    h1_real_pred_train  =  doAna(df_data = df_data_train, tag='train')
    h1_real_pred_test   =  doAna(df_data = df_data_test , tag='test' )
    str_sig = '#nu0bb'
    str_bkg = 'e^{-}'
    leg_train = 'hist'
    leg_test  = 'pel'
    ...
    hs_dict_pred = {}
    hs_dict_pred[str_sig ]=[h1_real_pred_train[2] ,2,leg_train]
    hs_dict_pred[str_bkg ]=[h1_real_pred_train[3] ,4,leg_test ]
    plot_hs(hs_dict=hs_dict_pred, norm=True, out_name=h1_real_pred_train[2].GetName(), title={'X':h1_real_pred_train[2].GetTitle(),'Y':'Events_{Normalized}'}, rangeX=[2.3,2.4], x_sep=[])
    hs_dict_pred = {}
    hs_dict_pred[str_sig ]=[h1_real_pred_train[4] ,2,leg_train]
    hs_dict_pred[str_bkg ]=[h1_real_pred_train[5] ,4,leg_test ]
    plot_hs(hs_dict=hs_dict_pred, norm=True, out_name=h1_real_pred_train[4].GetName(), title={'X':h1_real_pred_train[4].GetTitle(),'Y':'Events_{Normalized}'}, rangeX=[0,16], x_sep=[])
