...
def draw_sim (tree, max_evt):
    ...
    h_dict['gr_sim_xy_nonckori'] = []
    N_gr = 10
    for i in range(N_gr):
        h_dict['gr_sim_xy_ckori'].append(rt.TGraph())
        h_dict['gr_sim_xy_nonckori'].append(rt.TGraph())
        ...
        if parse_args.draw_xy_ck and tmp_gen_r < parse_args.gen_r_cut and  simE > 2.35 and simE < 2.38 and gr_index < len(h_dict['gr_sim_xy_ckori']):
            for ih in range(len(simhit_PMTIDs)):
                ...
                if simhit_isCerenkov[ih] == 1 and simhit_isOriginalOP[ih] == 1:
                    ...
                    h_dict['gr_sim_xy_nonckori'][gr_index].SetPoint(h_dict['gr_sim_xy_nonckori'][gr_index].GetN(), tmp_pmt_v3.X(), tmp_pmt_v3.Y())
            gr_index += 1 
        if len(initX)==2:
...
def root2hdf5_Points_detsim_v3 (batch_size, tree, out_name):##add 1D hist of hittime_cor
    ...
    for ie in m_evt_list:
        ...
        gen_p_v3  = rt.TVector3(initPX,initPY,initPZ)
        if parse_args.smear_gen:
            gen_v3_bak = rt.TVector3(initPX,initPY,initPZ)
            normal_vector = gen_p_v3.Cross(ref_vector)
            smear_costh = m_h_smear.GetRandom() 
            #print('smear_costh=',smear_costh)
            smear_angle = math.acos(smear_costh)
            rand_angle  = np.random.uniform(0,2*math.pi,1)[0]
            gen_p_v3.Rotate(smear_angle, normal_vector); # rotation around normal_vector
            gen_p_v3.Rotate(rand_angle , gen_v3_bak   ); # rotation around gen_v3_bak
...
def do_plot_gr2(gr1,gr2,out_name,title):
    canvas=rt.TCanvas("%s"%(out_name),"",800,800)
    canvas.cd()
    canvas.SetTopMargin(0.13)
    canvas.SetBottomMargin(0.1)
    ...
    gr1.GetXaxis().SetTitle(title['X'])
    gr1.GetYaxis().SetTitle(title['Y'])
    ...
    legend.AddEntry(gr1 ,'ck_ori','p')
    legend.AddEntry(gr2 ,'all ph','p')
    legend.SetBorderSize(0)
    legend.SetFillStyle(0)
    legend.SetTextSize(0.03)
    legend.SetTextFont(42)
...
if __name__ == '__main__':
    ...
    if parse_args.DoPlot:
        ...
        for i in range(len(h_dict['gr_sim_xy_ckori'])):
            ...
            do_plot_gr2(gr1=h_dict['gr_sim_xy_ckori'][i],gr2=h_dict['gr_sim_xy_nonckori'][i],out_name='gr2_sim_xy_rcut%.1f_idx%d'%(parse_args.gen_r_cut,i),title={'X':'x(mm)','Y':'y(mm)'})
